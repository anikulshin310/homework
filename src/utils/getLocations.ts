...
// Плюс по типизации: в целом, не очень хорошо, когда приходится undefined буквально указывать, такое редко должно быть, и в целом надо этого избегать. В таком случае обычно null передают, но если ты хочешь сделать параметр необязательным для функции, то напиши вот так: async (lat?: number, lon?: number, name = '') => ...
